...
fork `forkToolUse=true` 到悬挂 tool_use 后，`handleFork` 预填 FlowRunState `pendingToolPermissions`（UI 展示卡片），但 executor 内部 Map 为空（lazy executor 未经 `canUseTool`）。用户在卡片作答后仍走统一 `flow.command.toolPermissionResult → FlowRunner.handleToolPermissionResult → executor.answerToolPermission` 路径。`answerToolPermission` 在 Map 未命中时进入兜底分支（`disposed` / `completed` 时直接 return）：
...
- fork tool_use 产生的悬挂 tool_use 不经 `canUseTool`，由 `handleFork` 预填 FlowRunState `pendingToolPermissions`；用户作答仍走统一 `answerToolPermission` 入口，兜底分支构造 tool_result 推入输入流（echo + push），属同一套机制、非旁路。
