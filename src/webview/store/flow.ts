  ...
  return {
    ...
    init: () => {
      ...
      const onMessage = (msg: ExtensionToWebviewMessage) => {
        let notifySwitch: { flowId: string; agentId: string; agentName: string; flowName: string } | null = null
        immerSet((draft) => {
          if (msg.type === 'error') {
            console.error(msg)
          ...
          if (msg.type === 'flow.signal.flowStart') {
            ...
            if (draft.activeFlowId === flowId) {
              ...
              notifySwitch = { flowId, agentId: msg.data.agentId, agentName, flowName: flow?.name ?? '' }
            }
            return
          }
              ...
              if (data.output) {
                ...
                if (nextAgentId) {
                  ...
                  if (draft.activeFlowId === flowId) {
                    ...
                    notifySwitch = { flowId, agentId: nextAgentId, agentName: nextAgentName, flowName: flow?.name ?? '' }
                  }
                } else {
                  fs.phase = 'completed'
