...
export const ChatPanel: FC<Props> = ({ flowId, agentId, agentName }) => {
  ...
  const sessions: AgentSession[] =
    flowState?.sessions.filter((s) => s.agentId === agentId) ?? []
  ...
  const statusText =
    status === 'chatting' ? '生成中' :
    status === 'waiting-user' ? '等待输入' :
    status === 'preparing' ? '准备中' :
    status === 'completed' ? '已完成' :
    status === 'error' ? '出错' : '就绪'

  const statusColor =
    status === 'chatting' ? 'processing' :
    status === 'waiting-user' ? 'warning' :
    status === 'completed' ? 'success' :
    status === 'error' ? 'error' : 'default'

  return (
    <div className='flex h-[450px] w-[380px] flex-col bg-[#1e1e2e] rounded-lg overflow-hidden'>
      {/* Header */}
      <div className='flex items-center justify-between border-b border-[#45475a] px-3 py-2'>
        <div className='flex items-center gap-2'>
          ...
          <Tag color={statusColor} className='m-0 text-[10px]'>{statusText}</Tag>
        </div>
        {canInterrupt && (
          <Button
            type='text'
