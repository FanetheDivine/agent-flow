...
改动代码后必须运行 `npm run check-types && npm run lint` 校验类型与 lint。
...
- [docs/system-prompt.md](docs/system-prompt.md) — `buildAgentSystemPrompt` 区块顺序、XML 标签、shareValues 分层注入硬约束。
...
- [src/extension/FlowRunnerManager/FlowRunner/CodeExecutor.ts](src/extension/FlowRunnerManager/FlowRunner/CodeExecutor.ts) — code 节点执行器，注入 `input` / `values` / `runCommand` / `cwd` / `askUserQuestion` / `vscode`。
