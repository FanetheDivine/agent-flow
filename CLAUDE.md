...
webview 端：[App](src/webview/App.tsx) → `<AgentFlow>`（xyflow）+ 双 `<ChatDrawer>`（host run 用 `hostDrawer`,默认 700;子 run 用 `subAgentDrawer`,默认 540,覆盖在 host 之上）,状态收敛到 [useFlowStore](src/webview/store/flow.ts)。
...
- **双 Drawer 架构**：host 模式下 host run 用 `hostDrawer`、子 run 共用 `subAgentDrawer`(切换不同子 run 通过 ChatPanel `key=runId` 强制重挂载;sub 默认 540，host 默认 700，sub `zIndex=1010` > host `zIndex=1000`)。`store.openChatDrawer` 按 `agentId === HOST_AGENT_ID` 自动分流到 hostDrawer / subAgentDrawer；子 Drawer 关闭后 host Drawer 与其滚动状态保留。每个 Drawer 各自挂载一份 ChatInput，草稿独立(实现简化);视觉上 sub 覆盖在 host 上，关闭 sub 后 host 显露。
