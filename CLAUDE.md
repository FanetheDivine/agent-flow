...
领域定义与校验在 [src/common/index.ts](src/common/index.ts)。Flow 定义按作用域持久化到 `.agent-flows.json`：全局（`os.homedir()`）和项目（workspace root）；`FlowRunState` 仅内存（项目文件可选缓存用于崩溃恢复）,extension 端 [FlowRunStateManager](src/extension/FlowRunStateManager.ts) 镜像;UI 状态仅 webview。
...
Flow 分两个作用域持久化：全局（`~/.agent-flows.json`，用户级）和项目（`<workspaceRoot>/.agent-flows.json`，工作区级）。

- `Flow.project?: boolean` 是内存/UI 标记，标识 flow 属于项目作用域。不持久化进磁盘 flows 数组；extension 加载时根据来源文件设置，保存时 [stripFlowRuntimeFields](src/common/index.ts) 除后路由到对应文件
- `Flow.icon?: string` 是可持久化的图标标识，AI 可通过 `GetFlowJSONSchema` 设置
- `PersistedDataSchema` 扩展了可选 `flowRunStates`，仅项目文件使用；写入时机：`flow.signal.*` 触发 `flushMessages` 节流周期时写入，以及 `save` 事件时同步写入
- 加载合并：项目 flows 标 `project: true`、全局 flows 清除 `project` 字段后合并（全局在前、项目在后）；内存中有消息但磁盘不存在的 flow 保留；**默认 flow 注入时机**：全局与项目 flows 同时为空（文件不存在、合法 `{"flows":[]}` 均视为空）时才将 `defaultStore.flows` 作为全局 flows 注入；任一侧有 flow 则不注入
- 保存路由：extension 按 `flow.project` 分流，strip 后分别写入对应文件
- [PersistedDataController](src/extension/PersistedDataController/index.ts) 构造时接受文件路径，`globalStore()` / `projectStore(root)` 两个静态工厂
- 克隆/粘贴产生的新 flow 自动标 `project: true`；fork 继承源 flow 的 `project` 值（全局 flow fork → 无 project，项目 flow fork → project:true）
- 项目 flow 可通过 [SortableFlowItem](src/webview/components/FlowListPanel/SortableFlowItem.tsx) 的"设为全局"操作（GlobalOutlined）删除 `project` 字段并触发 save，之后 extension 将其路由到全局文件；全局 flow 不提供反向操作
