...
- **killFlow / clearFlow / flowStart 语义对照** → [flowRunState.ts](src/common/flowRunState.ts) `killFlow` / `clearFlow` / `flowStart` 分支:killFlow 保留 messages / shareValues 并投影 stopped;clearFlow 返回 undefined 彻底清空;flowStart 追加新 run 并保留 messages / shareValues;shareValues 仅在 phase→completed 时清空
