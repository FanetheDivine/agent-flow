...
- **host 模式硬约束**：
  - `HOST_AGENT_ID` 是保留 ID，普通 Agent 禁止使用；`flowStart.mode='host'` 时 `agentId` 必须是 `HOST_AGENT_ID`，FlowRunner 用 `buildHostSystemPrompt` + `buildHostMcpServer`(仅 `runAgent`)；`runAgent` 不能调度自身、不返回 newRunId。
  - `mode` 字段在 `killFlow` 后**保留**，仅下次 `flowStart` 才覆盖 —— 让用户停掉再重启时仍是 host 模式。
  - **host 子 run 禁止 fork**：MessageList 中 `isSubRun` 时把 `ctx.onFork = undefined`。子 run 的 fork 会让新 Flow 中子 run 找不到 host 接收 promise，造成 AgentComplete 无人接收死锁。
  - **runAgent toolUseId 队列法**：`makeRunAgentHandler` 必须先 `consumePendingRunAgentToolUseId` 出队，再做 `findAgentById` 校验；否则 id 错时 throw 但 toolUseId 已入队 → 下次正常调用 FIFO 取到错误残留，造成 `parentToolUseId` 错位。
  - **subAgentStarted 携带 initMessage / values**：FlowRunner.fire 时附带（reducer 据此把 host 注入的 message + values 序列化成两条 user aiMessage 写入子 run.messages 首屏），webview 端不需要再追加 echo。values 同时通过 systemPrompt 的「# 可用数据」节真正喂给子 SDK，run.messages 里的 values 块仅用于 UI 显示。
  - **子 run lazy resume**：host run interrupt 时 `cascadeKillSubExecutors` 把所有子 executor `kill + executors.delete`,reducer 端通过 `forceStopped` 把子 run 投影为 `stopped`。用户在 sub Drawer 继续向已 stopped 的子 run 发消息时,`FlowRunner.handleUserMessage` 找不到 executor → 走 `tryResumeSubRun`：通过 `getRunInfo(runId)` 拿到 sessionId / agentId / parentToolUseId,新建 lazy `ClaudeExecutor` 并 push 消息;reducer 在 `flow.command.userMessage` 上识别已 forceStopped 的 run 并清掉标记,getRunPhase 重新走 'running' 路径。子 run AgentComplete 时 `pendingRunAgentHandlers` 已被 reject,`onHostSubAgentComplete` 跳过 resolve 只 fire signal —— host AI 不会再次收到该子 run 的结果(host run 已 interrupt,无人接收)。
- **单 ChatDrawer 实例 + 双 state**：webview 仅挂一份 `<ChatDrawer>`，按 `drawerState = subAgentDrawer ?? hostDrawer` 选取 state（子 Drawer state 优先）；onClose 行为 `subAgentDrawer ? closeSubAgentDrawer() : closeHostDrawer()`，关 sub 后视觉自然回到 host。`store.openChatDrawer` 按 `agentId === HOST_AGENT_ID` 自动分流写入 hostDrawer / subAgentDrawer；切换不同子 run 时复用同一 Drawer 实例，通过 ChatPanel `key=${flowId}-${agentId}-${runId}` 强制重挂载。ChatInput 全局唯一(单 Drawer 实例 + `forceRender` 保证)，草稿在 host run / 子 run 切换时不丢。
- **host icon 入口分流**：[SortableFlowItem.onClickHostIcon](src/webview/components/FlowListPanel/SortableFlowItem.tsx) 先扫 `state.runs.filter(r => r.parentToolUseId)` 中处于 `awaiting-tool-permission` / `awaiting-question` / `result` / `error` 的子 run，命中则 `openSubAgentDrawer` + `destroyRunNotifications` 直接进该子 run，**不弹**通知；否则呼出 host Drawer。manual 模式非 idle 状态点 host icon 弹互斥通知，不启动。
- **HOST_AGENT_ID agentName 兜底**：reducer `pushEffect` 拼通知文案时识别 `agentId === HOST_AGENT_ID`，固定显示 `'AI 托管'`。host run agentId 不在 `flow.agents` 中，没兜底会显示空字符串。
